...
public:
  ...
  virtual void SetUp() {
    ...
    int width = 800;
    int height = 600;
    s_window = glfwCreateWindow(width, height, "test_UiConsole",
                                nullptr, nullptr);
    if (!s_window) {
...
TEST(Test_UIConsole, genericTests) {

  // Constructor
  Vaango_UIConsolePanel console;
  ...
  while (!glfwWindowShouldClose(Vaango_UIEnv::s_window)) {
    ...
    // Draw console
    console.draw("Output", console_width, console_height);
