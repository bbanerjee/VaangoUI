...
// Gizmo state
struct GizmoState {
    Qt3DCore::QTransform* pyramidTransform;
...
Qt3DCore::QEntity* createRotationGizmo(Qt3DCore::QEntity* parent, GizmoState* state)
{
    using namespace Qt3DCore;
    using namespace Qt3DExtras;
    using namespace Qt3DRender;

    QEntity* gizmoRoot = new QEntity(parent);

    // X-axis ring (red)
    QEntity* xRing = new QEntity(gizmoRoot);
    QTorusMesh* xTorus = new QTorusMesh();
    xTorus->setRadius(0.8f);
    xTorus->setMinorRadius(0.03f);
    xTorus->setRings(32);
    xTorus->setSlices(16);
    ...
    QPhongMaterial* xMaterial = new QPhongMaterial();
    xMaterial->setDiffuse(Qt::red);
    xMaterial->setAmbient(Qt::red);
    
    QObjectPicker* xPicker = new QObjectPicker();
    xPicker->setEnabled(true);
    
    xRing->addComponent(xTorus);
    xRing->addComponent(xTransform);
    xRing->addComponent(xMaterial);
    xRing->addComponent(xPicker);

    // Y-axis ring (green)
    QEntity* yRing = new QEntity(gizmoRoot);
    QTorusMesh* yTorus = new QTorusMesh();
    yTorus->setRadius(0.8f);
    yTorus->setMinorRadius(0.03f);
    yTorus->setRings(32);
    yTorus->setSlices(16);
    
    Qt3DCore::QTransform* yTransform = new Qt3DCore::QTransform();
    // Y is already vertical, no rotation needed
    
    QPhongMaterial* yMaterial = new QPhongMaterial();
    yMaterial->setDiffuse(Qt::green);
    yMaterial->setAmbient(Qt::green);
    
    QObjectPicker* yPicker = new QObjectPicker();
    yPicker->setEnabled(true);
    
    yRing->addComponent(yTorus);
    yRing->addComponent(yTransform);
    yRing->addComponent(yMaterial);
    yRing->addComponent(yPicker);

    // Z-axis ring (blue)
    QEntity* zRing = new QEntity(gizmoRoot);
    QTorusMesh* zTorus = new QTorusMesh();
    zTorus->setRadius(0.8f);
    zTorus->setMinorRadius(0.03f);
    zTorus->setRings(32);
    zTorus->setSlices(16);
    ...
    QPhongMaterial* zMaterial = new QPhongMaterial();
    zMaterial->setDiffuse(Qt::blue);
    zMaterial->setAmbient(Qt::blue);
    
    QObjectPicker* zPicker = new QObjectPicker();
    zPicker->setEnabled(true);
    
    zRing->addComponent(zTorus);
    zRing->addComponent(zTransform);
    zRing->addComponent(zMaterial);
    zRing->addComponent(zPicker);

    // Connect pickers to set active axis
    QObject::connect(xPicker, &QObjectPicker::pressed, [state, xMaterial]() {
        state->activeAxis = 0;
        xMaterial->setDiffuse(QColor(255, 150, 150)); // Lighter red
        std::cout << "X-axis selected" << std::endl;
    });
    
    QObject::connect(xPicker, &QObjectPicker::released, [state, xMaterial]() {
        state->activeAxis = -1;
        xMaterial->setDiffuse(Qt::red);
    });

    QObject::connect(yPicker, &QObjectPicker::pressed, [state, yMaterial]() {
        state->activeAxis = 1;
        yMaterial->setDiffuse(QColor(150, 255, 150)); // Lighter green
        std::cout << "Y-axis selected" << std::endl;
    });
    
    QObject::connect(yPicker, &QObjectPicker::released, [state, yMaterial]() {
        state->activeAxis = -1;
        yMaterial->setDiffuse(Qt::green);
    });

    QObject::connect(zPicker, &QObjectPicker::pressed, [state, zMaterial]() {
        state->activeAxis = 2;
        zMaterial->setDiffuse(QColor(150, 150, 255)); // Lighter blue
        std::cout << "Z-axis selected" << std::endl;
    });
    
    QObject::connect(zPicker, &QObjectPicker::released, [state, zMaterial]() {
        state->activeAxis = -1;
        zMaterial->setDiffuse(Qt::blue);
    });

    // Handle mouse move for rotation
    QObject::connect(xPicker, &QObjectPicker::moved, [state](Qt3DRender::QPickEvent* event) {
        if (state->activeAxis == 0) {
            QPointF pos = event->position();
            if (state->lastMousePos.isNull()) {
                state->lastMousePos = QVector2D(pos);
            } else {
                QVector2D delta = QVector2D(pos) - state->lastMousePos;
                state->currentRotation.setX(state->currentRotation.x() + delta.y() * 0.5f);
                state->pyramidTransform->setRotationX(state->currentRotation.x());
                state->lastMousePos = QVector2D(pos);
            }
        }
    });

    QObject::connect(yPicker, &QObjectPicker::moved, [state](Qt3DRender::QPickEvent* event) {
        if (state->activeAxis == 1) {
            ...
            if (state->lastMousePos.isNull()) {
                ...
                QVector2D delta = QVector2D(pos) - state->lastMousePos;
                state->currentRotation.setY(state->currentRotation.y() + delta.x() * 0.5f);
                state->pyramidTransform->setRotationY(state->currentRotation.y());
                state->lastMousePos = QVector2D(pos);
            }
        }
    });

    QObject::connect(zPicker, &QObjectPicker::moved, [state](Qt3DRender::QPickEvent* event) {
        if (state->activeAxis == 2) {
            QPointF pos = event->position();
            if (state->lastMousePos.isNull()) {
                state->lastMousePos = QVector2D(pos);
            } else {
                QVector2D delta = QVector2D(pos) - state->lastMousePos;
                state->currentRotation.setZ(state->currentRotation.z() + delta.x() * 0.5f);
                state->pyramidTransform->setRotationZ(state->currentRotation.z());
                state->lastMousePos = QVector2D(pos);
            }
        }
    });
...
int main(int argc, char** argv)
{
    ...
    // Input settings (required for picking)
    ...
    gizmoState->pyramidTransform = transform;
    gizmoState->currentRotation = QVector3D(0, 0, 0);
    createRotationGizmo(rootEntity, gizmoState);
